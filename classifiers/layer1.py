...
class L1_Classifier:
	...
	def __init__(self, config, verbose=False):
		'''Create a model object with given configuration options. Needs config options:'''

		self.verbose = verbose

		# get options
		self.attack_keys   = config.options('labels-l1')
		self.attacks	   = dict(zip(self.attack_keys, range(len(self.attack_keys))))
		...
		self.classifier_module = config.get('l1', 'classifier-module') if config.has_option('l1', 'classifier-module') else None
		...
		self.model = None # leave initialized (run self.train)
	...
	def predict(self, test_data):
		...
		print_stats(y_predicted, y_test, self.n_labels, self.outputs, lambda i: self.attack_keys[i])
